/**
 * Created by saaniaki on 05/09/17.
 */
...
function loadTabs(infoID, moduleID, cacheObj) {
    ...
    $.ajax({
        ...
        cache: cacheObj,
        success: function(data) {
            $(".js_dynamic-tab").remove();
            $(".mod-options div.tab-content").append(data);
            ...
            $('.dateButton .btn').tooltip({container: 'body'});
        }
    });
}
...
$(document).ready(function() {
    ...
    loadTabs($('.mod-options .mod-graph-type select').find('option:selected').attr('data-info-index'), moduleOptions.attr('data-module_id'));
